...
class TestCourierLogin:
    ...
    @allure.description('Авторизация курерьера с неправильным логином/паролем возвращает ' \
                        'ошибку 404 Not Found и "message": "Учетная запись не найдена"')
    ...
    def test_courier_cannot_login_with_error_login_or_password(self, create_courier, error_field):
        ...
        assert login_courier_response.status_code == 404 and login_courier_response.json()["message"] == 'Учетная запись не найдена'
   
    @allure.title('Курьер не может авторизоваться без поля логина или пароля')
    @allure.description('Авторизация курьера без поля логина или пароля возвращает ' \
                        'ошибку 400 Bad Request и "message": "Недостаточно данных для входа"')
    ...
    def test_courier_cannot_login_without_login_or_password(self, create_courier, missing_field):
        ...
        login_courier_response = CouriersMethod.login_courier(courier_data_for_login)
        assert login_courier_response.status_code == 400 and login_courier_response.json()["message"] == 'Недостаточно данных для входа'
